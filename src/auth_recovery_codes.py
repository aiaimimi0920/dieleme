"""Recovery protocol states and safe desktop messages; no runtime I/O."""

from types import MappingProxyType

ACTIVE_CODES = MappingProxyType(
    {
...
ACTIVE_STATUSES = frozenset(ACTIVE_CODES)
SNAPSHOT_AVAILABLE_STATUSES = frozenset({"snapshot_ready", "pc2_claimed", "restarting"})
TIMEOUT_REASONS = MappingProxyType(
    {status: f"{status}_timeout" for status in ACTIVE_CODES}
)
FAILURE_CODES = MappingProxyType(
    {
...
TERMINAL_CODE = "recovery_finished"
STAGE_VERIFY_CODE = "pc2_stage_verifying"
CHALLENGE_CHANGED_CODE = "challenge_changed"
UNKNOWN_RECOVERY_CODE = "recovery_unknown"
...
PHASE_MESSAGES = MappingProxyType(
    {
...
def active_code(status: object, *, scoped: bool = False) -> str | None:
    if not isinstance(status, str):
        return None
    if scoped and status == "restarting":
        return STAGE_VERIFY_CODE
    return ACTIVE_CODES.get(status)


def failure_code(reason: object) -> str:
    return (
        FAILURE_CODES.get(reason, TERMINAL_CODE)
