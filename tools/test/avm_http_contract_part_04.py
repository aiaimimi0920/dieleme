...
class AVMHttpContractPart04:
    ...
    def test_get_item_legacy_endpoint_rejects_missing_id(self):
        self._assert_http_error_code('/api/get_item', 400, 'AVM_INVALID_ID')

    def test_get_item_legacy_endpoint_returns_404_when_item_not_found(self):
        fake_repo = mock.Mock()
        fake_repo.enabled = True
        fake_repo.get_flat_item.return_value = None
        with mock.patch.object(server_module, 'DB_REPOSITORY', fake_repo):
            self._assert_http_error_code('/api/get_item?id=missing', 404, 'AVM_DETAIL_ITEM_NOT_FOUND')
    ...
    def test_get_or_create_sniff_task_legacy_endpoint(self):
        ...
        with mock.patch.object(server_module, '_seed_collection_service', return_value=fake_service):
            (status, payload) = self._post_json('/api/get_or_create_sniff_task', {'session_id': 's-1'})
        self.assertEqual(status, 200)
    ...
    def test_get_or_create_sniff_task_legacy_endpoint_returns_json_error_on_failure(self):
        ...
        with mock.patch.object(server_module, '_seed_collection_service', return_value=fake_service):
            with self.assertRaises(urllib.error.HTTPError) as ctx:
                self._open_post('/api/get_or_create_sniff_task', {'session_id': 's-1'})
        self.assertEqual(ctx.exception.code, 500)
        body = json.loads(ctx.exception.read().decode('utf-8'))
    ...
    def test_get_next_task_legacy_endpoint(self):
        with mock.patch.object(server_module, '_prefer_db_task_reads', return_value=True), mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            ...
            mocked_factory.return_value = fake_service
            request = urllib.request.Request(f'http://127.0.0.1:{self.port}/api/get_next_task', data=b'', headers={'Content-Type': 'application/json', 'X-FAPAI-Control-Token': 'test-operator'}, method='POST')
    ...
    def test_start_all_subtasks_endpoint_queues_tracked_work_with_defaults(self):
        expected_result = {'status': 'completed', 'source': 'start-all'}
        req = urllib.request.Request(f'http://127.0.0.1:{self.port}/api/avm/start_all_subtasks', data=b'', headers={'X-FAPAI-Control-Token': 'test-operator'}, method='POST')
        with mock.patch.object(server_module.AVM_PIPELINE, 'run', return_value=expected_result) as mocked_run:
            with urllib.request.urlopen(req) as resp:
                ...
                status = resp.status
            payload = self._complete_collection_job(status, accepted)
        self.assertEqual(status, 202)
        self.assertEqual(payload, expected_result)
        mocked_run.assert_called_once()
        (_, kwargs) = mocked_run.call_args
        self.assertFalse(kwargs['async_mode'])
        self.assertEqual(kwargs['config'].data_dir, str(Path(self.data_dir).resolve()))
    ...
    def test_start_all_subtasks_endpoint_returns_json_error_on_failure(self):
        ...
        with mock.patch.object(server_module.AVM_PIPELINE, 'run', side_effect=RuntimeError('boom')):
            with urllib.request.urlopen(req) as resp:
                accepted = json.loads(resp.read().decode('utf-8'))
            self._assert_collection_job_failed(resp.status, accepted, 'AVM_START_ALL_SUBTASKS_FAILED')

    def test_run_all_subtasks_sync_endpoint_runs_sync_defaults(self):
        expected_result = {'status': 'completed', 'source': 'sync-all'}
        req = urllib.request.Request(f'http://127.0.0.1:{self.port}/api/avm/run_all_subtasks_sync', data=b'', headers={'X-FAPAI-Control-Token': 'test-operator'}, method='POST')
        with mock.patch.object(server_module.AVM_PIPELINE, 'run', return_value=expected_result) as mocked_run:
            with urllib.request.urlopen(req) as resp:
                accepted = json.loads(resp.read().decode('utf-8'))
                status = resp.status
    ...
    def test_run_all_subtasks_sync_endpoint_returns_json_error_on_failure(self):
        ...
        with mock.patch.object(server_module.AVM_PIPELINE, 'run', side_effect=RuntimeError('boom')):
            with urllib.request.urlopen(req) as resp:
                accepted = json.loads(resp.read().decode('utf-8'))
            self._assert_collection_job_failed(resp.status, accepted, 'AVM_RUN_ALL_SUBTASKS_SYNC_FAILED')
    ...
    def test_release_gate_endpoint_defaults_invalid_numeric_body_fields(self):
        ...
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload) as mocked_report, mock.patch.object(server_module, '_avm_operator_eval_summary', return_value={}):
            (status, payload) = self._post_collection_job('/api/avm/release_gate', {'window_days': 'bad', 'min_sample_size': 'bad', 'smoke_sample_size': 'bad'})
        self.assertEqual(status, 202)
        self.assertTrue(payload['api_smoke'].get('skipped'))
    ...
    def test_release_gate_endpoint_clamps_negative_numeric_body_fields(self):
        gate_payload = {'pass': False, 'evaluation': {'pass': False}, 'completeness': {'pass': True}, 'drift': {'pass': True}, 'api_smoke': {'skipped': True}}
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload) as mocked_report, mock.patch.object(server_module, '_avm_operator_eval_summary', return_value={}):
            (status, payload) = self._post_collection_job('/api/avm/release_gate', {'window_days': '-1', 'min_sample_size': '-1', 'smoke_sample_size': '-1'})
        self.assertEqual(status, 202)
        self.assertTrue(payload['api_smoke'].get('skipped'))
        mocked_report.assert_called_once()
        self.assertEqual(mocked_report.call_args.kwargs['window_days'], 7)
    ...
    def test_release_gate_endpoint_returns_json_error_on_report_generation_failure(self):
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', side_effect=RuntimeError('boom')):
            with self._open_post('/api/avm/release_gate', {'window_days': '7', 'min_sample_size': '1', 'smoke_sample_size': '0'}) as response:
                self._assert_collection_job_failed(response.status, json.load(response), 'AVM_RELEASE_GATE_FAILED')
    ...
    def test_analysis_release_gate_endpoint_defaults_invalid_numeric_body_fields(self):
        gate_payload = {'pass': False, 'evaluation': {'pass': False}, 'completeness': {'pass': True}, 'drift': {'pass': True}, 'api_smoke': {'skipped': True}}
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload) as mocked_report, mock.patch.object(server_module, '_avm_operator_eval_summary', return_value={}):
            (status, payload) = self._post_collection_job('/api/analysis/release_gate', {'window_days': 'bad', 'min_sample_size': 'bad', 'smoke_sample_size': 'bad'})
        self.assertEqual(status, 202)
        self.assertTrue(payload['api_smoke'].get('skipped'))
        mocked_report.assert_called_once()
        self.assertEqual(mocked_report.call_args.kwargs['window_days'], 7)
    ...
    def test_analysis_release_gate_endpoint_clamps_negative_numeric_body_fields(self):
        gate_payload = {'pass': False, 'evaluation': {'pass': False}, 'completeness': {'pass': True}, 'drift': {'pass': True}, 'api_smoke': {'skipped': True}}
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload) as mocked_report, mock.patch.object(server_module, '_avm_operator_eval_summary', return_value={}):
            (status, payload) = self._post_collection_job('/api/analysis/release_gate', {'window_days': '-1', 'min_sample_size': '-1', 'smoke_sample_size': '-1'})
        self.assertEqual(status, 202)
        self.assertTrue(payload['api_smoke'].get('skipped'))
        mocked_report.assert_called_once()
        self.assertEqual(mocked_report.call_args.kwargs['window_days'], 7)
