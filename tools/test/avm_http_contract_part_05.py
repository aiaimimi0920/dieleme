...
class AVMHttpContractPart05:
    def test_release_gate_endpoint_tolerates_malformed_config_file(self):
        ...
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload):
            (status, payload) = self._post_collection_job('/api/avm/release_gate', {'window_days': '7', 'min_sample_size': '1', 'smoke_sample_size': '0'})
    ...
    def test_analysis_release_gate_endpoint_preserves_analysis_readiness_and_flattens_calibration_summary(self):
        ...
        with mock.patch('tools.avm_release_gate.generate_release_gate_report', return_value=gate_payload):
            (status, payload) = self._post_collection_job('/api/analysis/release_gate', {'window_days': '7', 'min_sample_size': '1', 'smoke_sample_size': '0'})
        self.assertEqual(status, 202)
    ...
    def test_recent_gap_audit_endpoint(self):
        (status, payload) = self._post_collection_job('/api/avm/recent_gap_audit', {'window_days': '7', 'sample_limit': '5'})
        self.assertEqual(status, 202)
        self.assertIn('record_count', payload)
        self.assertIn('missing_field_counts', payload)
        self.assertIn('samples', payload)

    def test_recent_gap_audit_endpoint_defaults_invalid_numeric_body_fields(self):
        ...
        with mock.patch('tools.audit_recent_avm_gaps.build_recent_gap_audit', return_value=mocked_output) as mocked_audit:
            (status, payload) = self._post_collection_job('/api/avm/recent_gap_audit', {'window_days': 'bad', 'sample_limit': 'bad'})
        self.assertEqual(status, 202)
    ...
    def test_recent_gap_audit_endpoint_clamps_negative_numeric_body_fields(self):
        mocked_output = {'record_count': 0, 'missing_field_counts': {}, 'samples': []}
        with mock.patch('tools.audit_recent_avm_gaps.build_recent_gap_audit', return_value=mocked_output) as mocked_audit:
            (status, payload) = self._post_collection_job('/api/avm/recent_gap_audit', {'window_days': '-1', 'sample_limit': '-1'})
        self.assertEqual(status, 202)
        self.assertEqual(payload['record_count'], 0)
        mocked_audit.assert_called_once()
        self.assertEqual(mocked_audit.call_args.kwargs['window_days'], 7)
    ...
    def test_recent_gap_audit_endpoint_returns_json_error_on_failure(self):
        with mock.patch('tools.audit_recent_avm_gaps.build_recent_gap_audit', side_effect=RuntimeError('boom')):
            with self._open_post('/api/avm/recent_gap_audit', {'window_days': '7', 'sample_limit': '5'}) as response:
                self._assert_collection_job_failed(response.status, json.load(response), 'AVM_RECENT_GAP_AUDIT_FAILED')
    ...
    def test_recent_detail_replay_endpoint(self):
        ...
        (status, payload) = self._post_collection_job('/api/avm/recent_detail_replay', {'window_days': '7', 'limit': '10', 'dry_run': False})
        self.assertEqual(status, 202)
    ...
    def test_recent_detail_replay_endpoint_returns_json_error_on_failure(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            ...
            status, accepted = self._post_json('/api/avm/recent_detail_replay', {'window_days': '7', 'limit': '10', 'dry_run': True})
            self._assert_collection_job_failed(status, accepted, 'AVM_RECENT_DETAIL_REPLAY_FAILED')

    def test_recent_detail_replay_post_body_endpoint_defaults_invalid_numeric_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 7, 'limit': 100, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/avm/recent_detail_replay', {'window_days': 'bad', 'limit': 'bad', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
    ...
    def test_recent_detail_replay_post_body_endpoint_clamps_negative_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 7, 'limit': 0, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/avm/recent_detail_replay', {'window_days': '-1', 'limit': '-1', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
        self.assertEqual(payload['window_days'], 7)
        self.assertEqual(payload['limit'], 0)
        fake_service.prepare_replay.assert_called_once_with(window_days=7, limit=0, dry_run=True)

    def test_collection_detail_prepare_replay_post_body_alias_endpoint(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 7, 'limit': 10, 'dry_run': True, 'prepared_count': 1}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/collection/details/prepare_replay', {'window_days': '7', 'limit': '10', 'dry_run': True})
        self.assertEqual(status, 202)
        self.assertEqual(payload['prepared_count'], 1)
        fake_service.prepare_replay.assert_called_once_with(window_days=7, limit=10, dry_run=True)

    def test_collection_detail_prepare_replay_post_body_alias_defaults_invalid_numeric_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 30, 'limit': 500, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/collection/details/prepare_replay', {'window_days': 'bad', 'limit': 'bad', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
    ...
    def test_collection_detail_prepare_replay_post_body_alias_clamps_negative_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 30, 'limit': 0, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/collection/details/prepare_replay', {'window_days': '-1', 'limit': '-1', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
        self.assertEqual(payload['window_days'], 30)
        self.assertEqual(payload['limit'], 0)
        fake_service.prepare_replay.assert_called_once_with(window_days=30, limit=0, dry_run=True)

    def test_collection_detail_prepare_replay_post_body_alias_returns_json_error_on_failure(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.side_effect = RuntimeError('boom')
            mocked_factory.return_value = fake_service
            status, accepted = self._post_json('/api/collection/details/prepare_replay', {'window_days': '7', 'limit': '10', 'dry_run': True})
            self._assert_collection_job_failed(status, accepted, 'AVM_ARCHIVE_DETAIL_REPLAY_FAILED')

    def test_archive_detail_replay_post_body_endpoint(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 30, 'limit': 11, 'dry_run': True, 'prepared_count': 2}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/avm/archive_detail_replay', {'window_days': '30', 'limit': '11', 'dry_run': True})
        self.assertEqual(status, 202)
    ...
    def test_archive_detail_replay_post_body_endpoint_returns_json_error_on_failure(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.side_effect = RuntimeError('boom')
            mocked_factory.return_value = fake_service
            status, accepted = self._post_json('/api/avm/archive_detail_replay', {'window_days': '30', 'limit': '11', 'dry_run': True})
            self._assert_collection_job_failed(status, accepted, 'AVM_ARCHIVE_DETAIL_REPLAY_FAILED')

    def test_archive_detail_replay_post_body_endpoint_defaults_invalid_numeric_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 30, 'limit': 500, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/avm/archive_detail_replay', {'window_days': 'bad', 'limit': 'bad', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
        self.assertEqual(payload['window_days'], 30)
        self.assertEqual(payload['limit'], 500)
        fake_service.prepare_replay.assert_called_once_with(window_days=30, limit=500, dry_run=True)

    def test_archive_detail_replay_post_body_endpoint_clamps_negative_body_fields(self):
        with mock.patch.object(server_module, '_detail_collection_service') as mocked_factory:
            fake_service = mock.Mock()
            fake_service.prepare_replay.return_value = {'window_days': 30, 'limit': 0, 'dry_run': True, 'prepared_count': 0}
            mocked_factory.return_value = fake_service
            (status, payload) = self._post_collection_job('/api/avm/archive_detail_replay', {'window_days': '-1', 'limit': '-1', 'dry_run': 'maybe'})
        self.assertEqual(status, 202)
        self.assertEqual(payload['window_days'], 30)
        self.assertEqual(payload['limit'], 0)
        fake_service.prepare_replay.assert_called_once_with(window_days=30, limit=0, dry_run=True)
